...
export const TenantSettingsPanel = () => {
  // ✅ CORRIGIDO: Ler da chave correta em sessionStorage
  const tenantId = sessionStorage.getItem('sb-auth-tenant-id') || localStorage.getItem('admin-tenant-id') || '';
  const { settings, isLoading, error, updateSettings, isUsingDefaults } = useTenantSettings(tenantId);

  // 🔍 DEBUG
  console.log('[TenantSettingsPanel] tenantId:', tenantId);
  console.log('[TenantSettingsPanel] settings:', settings);
  console.log('[TenantSettingsPanel] isLoading:', isLoading);
  console.log('[TenantSettingsPanel] error:', error);
  console.log('[TenantSettingsPanel] isUsingDefaults:', isUsingDefaults);

  const [form, setForm] = useState<any>({});
  const [isDirty, setIsDirty] = useState(false);
  const [isSaving, setIsSaving] = useState(false);

  // Sincronizar settings quando carregarem (com ou sem defaults)
  useEffect(() => {
    if (settings) {
      setForm(settings);
      setIsDirty(false);
  ...
  // ✅ NOVO: Se isLoading E ainda não tem settings, mostrar spinner
  ...
  // ✅ NOVO: Se tem settings (BD, RPC, ou defaults), SEMPRE mostrar UI
  // NUNCA mostrar spinner ou erro depois que settings é carregado
  if (!settings) {
    // This should NEVER happen because hook always sets defaults
    // But just in case, show loading state
    return (
      <Card>
        <CardHeader>
          <CardTitle>Configurações da Loja</CardTitle>
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Customize sua loja e ative/desative recursos
              {isUsingDefaults && (
                <span className="ml-2 text-xs text-orange-600">
                  (usando valores padrão - salve para finalizar setup)
                </span>
              )}
            </CardDescription>
          </div>
