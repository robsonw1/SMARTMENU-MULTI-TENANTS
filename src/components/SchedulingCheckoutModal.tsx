...
// 5-step flow: contact, delivery, address, scheduling, payment (WITH scheduling)
const getVisibleSteps = (deliveryType: string): Step[] => {
  let steps: Step[] = ['contact', 'delivery', 'address', 'scheduling', 'payment'];
...
export function SchedulingCheckoutModal() {
  const { isSchedulingCheckoutOpen, setSchedulingCheckoutOpen, setCartOpen } = useUIStore();
  ...
  useEffect(() => {
    if (isSchedulingCheckoutOpen && currentCustomer && isRemembered) {
  ...
  useEffect(() => {
    if (isSchedulingCheckoutOpen && currentCustomer?.street && !address.street) {
      setAddress({
    ...
    } else if (isSchedulingCheckoutOpen && !currentCustomer) {
  ...
  useEffect(() => {
    if (!isSchedulingCheckoutOpen || currentCustomer || !customer.name || step !== 'contact') return;
  ...
  }, [customer.name, customer.phone, customer.email, isSchedulingCheckoutOpen, currentCustomer, step]);
  ...
  useEffect(() => {
    if (!isSchedulingCheckoutOpen || !currentCustomer?.id) return;
  ...
  }, [isSchedulingCheckoutOpen, currentCustomer?.id, pointsToRedeem, currentCustomer?.totalPoints]);
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      .channel('orders-scheduling-pix-confirm')
      .on('postgres_changes',
  ...
  const handleAddNewNeighborhood = async () => {
    ...
    try {
      ...
      console.log('[SCHEDULING] Criando bairro via Edge Function com tenant_id:', tenantIdFromStorage);

      if (!tenantIdFromStorage) {
        console.error('❌ [SCHEDULING] tenant_id não encontrado no sessionStorage');
      ...
      console.log('✅ [SCHEDULING] Bairro criado com sucesso via Edge Function:', responseData);
  ...
  const validateStep = (currentStep: Step): boolean => {
    switch (currentStep) {
      ...
      case 'address':
        ...
        return true;
      case 'scheduling':
        // REQUIRED: scheduling must have date and time
      ...
      if (paymentMethod === 'pix') {
        ...
        const { data: mpData, error: mpError } = await supabase.functions.invoke('mercadopago-payment', {
          body: {
            ...
            payerEmail: 'robsonw.negocios@gmail.com',
